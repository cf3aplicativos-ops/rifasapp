...
export async function loginAction(formData: FormData) {
  ...
  try {
    await signIn("credentials", {
      ...
      tenantId: tenant.id,
      // "/vendedores/dashboard" (no "/dashboard"): a diferencia de
      // `redirect()` de next/navigation, el `redirectTo` de Auth.js NO
      // hereda el basePath solo — confirmado en la práctica (Fase 13,
      // Multi Zones).
      redirectTo: "/vendedores/dashboard",
    });
  } catch (error) {
